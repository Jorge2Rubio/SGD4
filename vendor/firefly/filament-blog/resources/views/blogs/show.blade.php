    ...
    <section class="pb-16">
        <div class="container mx-auto">
            <div class="mb-10 flex gap-x-2 text-sm font-semibold">
                ...
                <a href="{{ route('filamentlessons.post.all') }}" class="opacity-60">Lessons</a> <!-- changed here -->
                <span class="opacity-30">/</span>
                                    ...
                                    <div class="pt-10">
                                        ...
                                        <div class="space-x-2 space-y-1">
                                            @foreach ($post->tags as $tag)
                                                <a href="{{ route('filamentlessons.tag.post', ['tag' => $tag->slug]) }}" class="rounded-full border border-slate-300 px-3 py-1 text-sm font-medium text-black text-slate-600 hover:bg-slate-100">
                                                    {{ $tag->name }}
                                                </a>
                                            @endforeach
                                        </div>
                                        ...
                                        <div class="mb-4 flex items-center gap-4">
                                            ...
                                            <div>
                                                <span class="block max-w-[150px] overflow-hidden text-ellipsis whitespace-nowrap font-semibold">
                                                    {{ $comment->user->{config('filamentlessons.user.columns.name')} }}
                                                </span>
                ...
                <div>
                    <div class="relative mb-6 flex items-center gap-x-8">
                        <h2 class="whitespace-nowrap text-xl font-semibold">
                            <span class="text-primary font-bold">#</span> Related Lessons
                        </h2>
                    ...
                    <div class="grid md:grid-cols-3 sm:grid-cols-1 gap-x-12 gap-y-10">
                        ...
                        <div class="col-span-3">
                            <p class="text-center text-xl font-semibold text-gray-300">No related lessons found.</p>
                        </div>
                        @endforelse
                    </div>

                    <div class="flex justify-center pt-20">
                        <a href="{{ route('filamentlessons.post.all') }}" class="flex items-center justify-center md:gap-x-5 rounded-full bg-slate-100 px-20 py-4 text-sm font-semibold transition-all duration-300 hover:bg-slate-200">
                            <span>Show all lessons</span> <!-- changed here -->
